...
export const createMenus = async (sequelize: Sequelize) => {
	...
	try {
		...
		const getDailyEventId = (name: string) => {
			...
			if (!event)
				throw new Error(`Evento diário não encontrado para associação com o menu: ${name}`)
			return event.id
		}
		...
		const menus = [
			{
				...
				dishes: ['PF de Frango Tradicional', 'Macarrão à Bolonhesa', 'Salada Vegana Proteica'],
			},
			{
				activationDate: new Date(),
			...
			},
			{
				...
				dishes: ['Omelete com legumes', 'Quinoa com legumes', 'Lentilha com arroz e salada'],
			},
			{
				activationDate: new Date(),
			...
			},
			{
				...
				dishes: ['Sobremesa de banana da casa', 'Sanduiche Leve', 'Wrap vegetariano'],
			},
			{
				activationDate: new Date(),
				availableDay: WEEK_DAYS.TUESDAY,
