...
export class DishDto {
	...
	@ApiProperty({
		description:
			'Lista de alimentos associados ao prato, com suas quantidades e valores nutricionais ajustados.',
		type: [DishFoodDto],
	})
	foods: DishFoodDto[]
	...
	static fromEntity(dish: Dish): DishDto {
		const foods =
			dish.foods?.map((food: any) => {
				...
				return new DishFoodDto(food.id, food.name, adjustedNutritionFacts, quantity)
			}) ?? []
